...
GT911_Status_t GT911_I2C_Write(uint8_t Addr, uint8_t *write_data, uint16_t write_length) {
	...
	// Use 10ms timeout instead of HAL_MAX_DELAY to prevent blocking audio
	switch (HAL_I2C_Master_Transmit(&hi2c3, Address, write_data, write_length, 10)) {
	case HAL_OK:
		return GT911_OK;
		break;
	case HAL_ERROR:
...
GT911_Status_t GT911_I2C_Read(uint8_t Addr, uint8_t* read_data, uint16_t read_length){
	uint8_t Address = Addr << 1;
	// Use 10ms timeout instead of HAL_MAX_DELAY to prevent blocking audio
	switch (HAL_I2C_Master_Receive(&hi2c3, Address, read_data, read_length, 10)) {
	case HAL_OK:
		return GT911_OK;
		break;
	case HAL_ERROR:
